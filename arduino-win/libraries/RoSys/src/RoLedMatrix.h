 ...
 * @版本：  V0.1.0
 * @时间：  2016/08/15
 * @描述：  16*16 LED点阵模块的驱动。RoLedMatrix.cpp 的头文件。
 *
 * \说明
 ...
 * \方法列表
 * 
 *    1.    void 	RoLedMatrix::setPin(int16_t dataPin, int16_t clkPin, int16_t csPin, int16_t Num);
 *    2.    void 	RoLedMatrix::setPin(uint8_t port, int16_t Num); 
 *    3.    int16_t RoLedMatrix::getDeviceCount();
 *    4.    void 	RoLedMatrix::shutDown(int16_t addr, bool _status);
 *    5.    void 	RoLedMatrix::setScanLimit(int16_t addr, int16_t limit);
 *    6.    void 	RoLedMatrix::setIntensity(int16_t addr, int16_t intensity);
 *    7.    void 	RoLedMatrix::setDisplayType(int8_t angleValue,int8_t sizeValue);
 *
 *    8.    void 	RoLedMatrix::clearDisplay(int16_t addr);
 *    9.    void 	RoLedMatrix::setLed(int16_t addr, int16_t row, int16_t col, boolean state);
 *    10.    void 	RoLedMatrix::setRow(int16_t addr, int16_t row, uint8_t value);
 *    11.   void 	RoLedMatrix::setColumn(int16_t addr, int16_t col, uint8_t value);
 *    12.   void 	RoLedMatrix::display(uint8_t addr,uint8_t ch) ;
 *    13.   void 	RoLedMatrix::display(uint8_t addr,uint8_t ch[]) ;
 *    14.   void 	RoLedMatrix::display(uint8_t addr,uint8_t (*FData)[8]);
 *
 *    15.   void 	RoLedMatrix::spiTransfer(int16_t addr, uint8_t opcode, uint8_t data);
 *
 * \修订历史
 * `<Author>`      `<Time>`        `<Version>`        `<Descr>`
 *  KING            2016/08/15      0.1.0              新建库文件。
 *  
 * \示例
 *  
 *    1.   DisplayTest.ino
...
#ifndef RoLedMatrix_h
#define RoLedMatrix_h
...
class RoLedMatrix 
{
    public:
		...
		*/
		 ...
		 * \其他：无
		 */	
        RoLedMatrix(int16_t dataPin, int16_t clkPin, int16_t csPin, int16_t Num=1);
		 ...
		 * \其他：无
		 */	
		RoLedMatrix(uint8_t port, int16_t Num=1);
