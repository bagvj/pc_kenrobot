 ...
 *    1.begin(void)
 *    2.setPin(uint8_t p)
 *    3.setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b)
 *    4.show(void)
 *
 * \修订历史
...
uper_RGB::uper_RGB(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), pixels(NULL)
  ,type(t)
#ifdef __AVR__
...
void uper_RGB::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void uper_RGB::show(void) {
...
void uper_RGB::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void uper_RGB::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t uper_RGB::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t uper_RGB::getPixelColor(uint16_t n) const {
...
uint8_t *uper_RGB::getPixels(void) const {
  return pixels;
}

uint16_t uper_RGB::numPixels(void) const {
  return numLEDs;
}
...
void uper_RGB::setBrightness(uint8_t b) {
