 ...
 *
 * \说明
 * uper_Heartrate心率传感器驱动函数
 *
 * \公有方法列表
 * 
 *      1.UPER_Heartrate(char mode) { mode_ = mode; }
 *      2.uint16_t getRate(void)
 *      3.uint8_t getCnt(void)
 *      4.uint16_t getValue(uint8_t pin)
...
* @return uint16_t
*/ 
uint16_t UPER_Heartrate::getValue(uint8_t pin) 
{
	valueCount_++;
...
* @return uint8_t
*/ 
uint8_t UPER_Heartrate::getCnt(void) 
{
    return(valueCount_);
}
...
char UPER_Heartrate::maxNumber(uint8_t count)
{
...
void UPER_Heartrate::minNumber(uint8_t count)
{
	uint16_t temp1,temp2;
	for(int i=0;i<4;i++){
		if(count<i){
...
uint16_t UPER_Heartrate::analogGetRate(void)
{
...
uint16_t UPER_Heartrate::digitalGetRate(void)
{
...
* @return uint16_t
*/ 
uint16_t UPER_Heartrate::getRate(void)
{
