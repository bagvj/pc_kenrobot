 ...
 *
 * \公有方法列表
 *    1.SDClass  boolean begin(uint8_t csPin = SD_CHIP_SELECT_PIN); //SD卡是否存在
 *    2.SDClass  File open(const String &filename, uint8_t mode = FILE_READ) { return open( filename.c_str(), mode ); };//打开文件名称和方式 读还是写
				打开一个没有的文件默认写参数的时候自动创建这个文件
 *    3.File     void close(); //关闭文件
 *	  4.File     read(); //读取SD卡某个文本内容
 				 写内容 myFile.println("testing 1, 2, 3.");
 *    5.SDClass  boolean exists(const String &filepath) { return exists(filepath.c_str());//是否存在某个文件
 *    6.SDClass  boolean remove(const char *filepath);//删除一个文件
