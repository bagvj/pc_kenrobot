 ...
 * @版本：  V0.1.0
 * @时间：  2017/07/21
 * @描述：  彩色LED灯模块、灯带驱动函数，RBGled.c 的头文件。
 *
 * \说明
 ...
 * \方法列表
 * 
 *    1. void RGBled::reset(uint8_t port)
 *    2. void RGBled::reset(uint8_t port,uint8_t slot)
 *    3. void RGBled::setPin(uint8_t port)
 *    4. uint8_t RGBled::getNumber()
 *    5. cRGB RGBled::getColorAt(uint8_t index)
 *    6. bool RGBled::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
 *    7. bool RGBled::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
 *    8. bool RGBled::setColor(uint8_t red, uint8_t green, uint8_t blue)
 *    9. bool RGBled::setColor(uint8_t index, long value)
 *    10. void RGBled::show()
 *
 * \修订历史
