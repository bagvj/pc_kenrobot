 ...
 * @名称：  Kenblock_Gamepad_Software.cpp
 * @作者：  Kenblock
 * @版本：  V0.4.5
 * @时间：  2018/1/30
 * @描述：  2.4G遥控手柄 Kenblock_Gamepad_Software.cpp。
 *
 * \说明
 * 2.4G 遥控手柄。
 *
...
#include "Kenblock_Gamepad_Software.h"
 ...
 Gamepad_Software::Gamepad_Software(SoftwareSerial &uart): m_puart(&uart)
 {
	 
 }


uint16_t Gamepad_Software::last_buttons = 0xffff;
uint16_t Gamepad_Software::buttons = 0xffff;
unsigned long Gamepad_Software::overtime = 0;
	...
	* \其他：无
	*/
void Gamepad_Software::begin(uint32_t baud)
{
    m_puart->begin(baud);
	//mySerial.begin(9600);
    rx_empty();
 ...
 * \说明： 清除串口接收缓存器中的值
 */
void Gamepad_Software::rx_empty(void) 
{
	...
	* \其他：无
	*/
void Gamepad_Software::dataRead(uint8_t id)
{	 
...
void Gamepad_Software::dataRead()
{	   
	...
	* \其他：无
	*/
 bool Gamepad_Software::NewButtonState()
{	
	...
	* \其他：无
	*/
bool Gamepad_Software::NewButtonState(unsigned int button)
{
	...
	* \其他：无
	*/
bool Gamepad_Software::ButtonPressed(unsigned int button)
{
	...
	* \其他：无
	*/
bool Gamepad_Software::ButtonReleased(unsigned int button)
{
	...
	* \其他：无
	*/
bool Gamepad_Software::Button(uint16_t button)
{	
	...
	* \其他：无
	*/
 uint8_t Gamepad_Software::Joystick(uint8_t joystick)
{	if(dataError)
	return 125;
	else
