 ...
 * \其他：无
 */	
RoLedMatrix::RoLedMatrix(int16_t dataPin, int16_t clkPin, int16_t csPin, int16_t Num) 
{
    SPI_MOSI= dataPin;
    SPI_CLK = clkPin;
    SPI_CS  = csPin;
 ...
 * \其他：无
 */	
RoLedMatrix::RoLedMatrix(uint8_t port, int16_t Num)
{
 ...
 * \其他：无
 */	
void RoLedMatrix::setPin(int16_t dataPin, int16_t clkPin, int16_t csPin, int16_t Num)
{
    SPI_MOSI= dataPin;
    SPI_CLK = clkPin;
    SPI_CS  = csPin;
 ...
 * \其他：无
 */	
void RoLedMatrix::setPin(uint8_t port, int16_t Num)
{
	SPI_MOSI = Ex_Quadruple_Digital[port].d1;
    SPI_CLK  = Ex_Quadruple_Digital[port].d3;
    SPI_CS   = Ex_Quadruple_Digital[port].d2;
 ...
 * \其他：无
 */
int16_t RoLedMatrix::getDeviceCount() 
{
    return _maxDevices;
}
 ...
 * \其他：无
 */
void RoLedMatrix::shutDown(int16_t addr, bool b) 
{
 ...
 * \其他：无
 */
void RoLedMatrix::setScanLimit(int16_t addr, int16_t limit) 
{
    if(addr<0 || addr>=_maxDevices)
        return;
    if(limit>=0 && limit<8)
 ...
 * \其他：无
 */
void RoLedMatrix::setIntensity(int16_t addr, int16_t intensity) 
{
    if(addr<0 || addr>=_maxDevices)
        return;
 ...
 * \其他：无
 */
void RoLedMatrix::setDisplayType(int8_t angleValue,int8_t sizeValue) 
{
 ...
 * \其他：无
 */
void RoLedMatrix::clearDisplay(int16_t addr) 
{
    int16_t offset;
 ...
 * \其他：无
 */
void RoLedMatrix::setLed(int16_t addr, int16_t row, int16_t column, bool state) 
{
    int16_t offset;
    uint8_t val=0x00;
 ...
 * \其他：无
 */
void RoLedMatrix::setRow(int16_t addr, int16_t row, uint8_t value) 
{
    int16_t offset;
    if(addr<0 || addr>= _maxDevices)
        return;
 ...
 * \其他：无
 */
void RoLedMatrix::setColumn(int16_t addr, int16_t col, uint8_t value) 
{
    uint8_t val;
...
void RoLedMatrix::spiTransfer(int16_t addr, volatile uint8_t opcode, volatile uint8_t data) 
{
 ...
 * \其他：无
 */
void RoLedMatrix::display(uint8_t addr,uint8_t (*FData)[8]) 
{
 ...
 * \其他：无
 */
void RoLedMatrix::display(uint8_t addr,uint8_t ch) 
{
