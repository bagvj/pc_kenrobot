 ...
 * \方法列表
 * 
 *    1. void RoRGBLed::reset(uint8_t port)
 *    2. void RoRGBLed::reset(uint8_t port,uint8_t slot)
 ...
 *   port - arduino port
 */
RoRGBLed::RoRGBLed(uint8_t port)
{
  _pinMask       = digitalPinToBitMask(port);
 ...
 */
RoRGBLed::RoRGBLed(uint8_t port, uint8_t led_num)
{
  _pinMask       = digitalPinToBitMask(port);
  ws2812_port   = portOutputRegister(digitalPinToPort(port) );
  // set pinMode OUTPUT */
 ...
 *   None
 */
void RoRGBLed::setPin(uint8_t port)
{
  _pinMask   = digitalPinToBitMask(port);
  ws2812_port = portOutputRegister(digitalPinToPort(port) );
  pinMode(port, OUTPUT);
 ...
 *   None
 */
void RoRGBLed::setNumber(uint8_t num_leds)
{
  count_led = num_leds;
 ...
 */
cRGB RoRGBLed::getColorAt(uint8_t index)
{
  cRGB px_value;
 ...
 *   The index value from 1 to the max
 */
uint8_t RoRGBLed::getNumber(void)
{
  return(count_led);
}
 ...
 */
bool RoRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
 ...
 */
bool RoRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index == 0)
  {
 ...
 */
bool RoRGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
{
 ...
 *   The index value from 1 to the max, if you set the index 0, all the LED will be lit
 */
bool RoRGBLed::setColor(uint8_t index, long value)
{
  if(index == 0)
  {
    for(int16_t i = 0; i < count_led; i++)
 ...
 *   None
 */
void RoRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
 ...
 *   None
 */
void RoRGBLed::show(void)
{
  rgbled_sendarray_mask(pixels, 3 * count_led, _pinMask, (uint8_t*)ws2812_port);
}
