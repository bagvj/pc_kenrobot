 ...
 *    6.   	void  	RoLedMatrix::display(uint8_t addr,uint8_t ch) ;
 *    7.   	void  	RoLedMatrix::display(uint8_t addr,uint8_t ch[]) ;
 *    8.   	void  	RoLedMatrix::display(uint8_t addr,uint8_t (*FData)[8]);
 * 
 * \修订历史
...
RoLedMatrix ledMatrix(PD6,2);  //16*16点阵模块接口（拓展板），模块数量设置为2
