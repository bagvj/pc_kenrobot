define(['./AtmelContext', './AtmelIO'], function(AtmelContext, AtmelIO) {
	...
	var ADC = {
		...
		Init: function() {
			...
			this.ADSCRA.OnRegisterChanged.push(function(oldVal, newVal) {
				self.ADSCRA_OnRegisterChanged(oldVal, newVal);
			});
			this.ADMUX = AtmelContext.RAM[AtmelIO.ADMUX];
			this.ADCH = AtmelContext.RAM[AtmelIO.ADCH];
			this.ADCH.OnRegisterRead.push(function(val) {
				return self.ADCH_OnRegisterRead(val);
			});
			this.ADCL = AtmelContext.RAM[AtmelIO.ADCL];
			this.ADCL.OnRegisterRead.push(function(val) {
				return self.ADCL_OnRegisterRead(val);
			});
		},
