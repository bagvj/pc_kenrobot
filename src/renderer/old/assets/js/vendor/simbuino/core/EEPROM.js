define(['./AtmelContext', './AtmelIO'], function(AtmelContext, AtmelIO) {
	var EEPROM = {
		Init: function() {
			...
			eecr.OnRegisterRead.push(function(val) {
				return self.EECR_OnRegisterRead(val);
			});
			eecr.OnRegisterChanged.push(function(oldVal, newVal) {
				self.EECR_OnRegisterChanged(oldVal, newVal);
			});
			var eedr = AtmelContext.RAM[AtmelIO.EEDR];
			eedr.OnRegisterRead.push(function(val) {
				return self.EEDR_OnRegisterRead(val);
			});
		},

		Clear: function() {
			for (var i = 0; i < AtmelContext.EEPROM.length; i++)
				AtmelContext.EEPROM[i] = 0xff;
		},
