define(['./AtmelContext', './AtmelIO'], function(AtmelContext, AtmelIO) {
	...
	var USART = {
		Init: function(UpdateInterruptFlags) {
			...
			this.UCSR0A.OnRegisterRead.push(function(val) {
				return self.UCSR0A_OnRegisterRead(val);
			});
			this.UCSR0A.OnRegisterChanged.push(function(oldVal, newVal) {
				self.UCSR0A_OnRegisterChanged(oldVal, newVal);
			});
			this.UCSR0B.OnRegisterChanged.push(function(oldVal, newVal) {
				self.UCSR0B_OnRegisterChanged(oldVal, newVal);
			});
			this.UDR0.OnRegisterChanged.push(function(oldVal, newVal) {
				self.UDR0_OnRegisterChanged(oldVal, newVal);
			});
		},
		...
		UCSR0A_OnRegisterChanged: function(oldVal, newVal) {
			...
			// {
			// 	if ((this.UCSR0A.Value & (1 << TXC0_BIT)) != 0)
			// 		this.UCSR0A.Value = newVal &= (1 << TXC0_BIT);
			// }
		},
